...
-- split lines in passphrases
-- for each passphrase:
     -- check that passphrase is made up of spaces and lower-case letters only. if not => invalid
     -- split words and check that there are no duplicate words. if not => invalid

input : String
input = """vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh
...
dec4a : String -> Nat
dec4a input = length (filter check $ lines input) where
  check : String -> Bool

  checkCharacters : String -> Bool
  checkCharacters str = let characters = unpack str in
    characters == filter  $ characters

  checkDuplicateWords : String -> Bool
